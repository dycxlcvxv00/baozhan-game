/* 爆战丨无限弹幕 v4.4 · BattleScene 渲染同步：战斗铺底 + 顶部血条 */
'use strict';
...
BattleScene.prototype.sync=function(){
  ...
  for(i=0;i<BZ.SLOT_POS.length;i++){var sp=BZ.SLOT_POS[i];this.hexBox(g,sp.x,sp.y,34,39,0x64748b,.72,0);this.hexBox(g,sp.x,sp.y,28,33,0x1e293b,1,0);}for(i=0;i<S.runes.length;i++){var r=S.runes[i],def=BZ.RUNE_DEFS[r.id],pct=Math.min(1,r.energy/def.energyMax);this.hexBox(g,r.x,r.y,34,39,def.color,.58,0);this.hexBox(g,r.x,r.y,28,33,def.color,pct>=1?.88:.55,0);if(r.flash>0)this.hex(g,r.x,r.y,21+r.flash*4,def.color,Math.max(0,r.flash),2);g.fillStyle(0x020617,1).fillRect(r.x-8,r.y+14,16,3);g.fillStyle(def.color,1).fillRect(r.x-8,r.y+14,16*pct,3);}
