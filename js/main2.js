/* 爆战丨无限弹幕 v2.0 · BattleScene 渲染同步（只读 sim 状态） */
'use strict';
...
BattleScene.prototype.sync = function () {
  ...
  for (i = 0; i < S.effects.length; i++) {
    ...
    else if (e.kind === 'shield') g.lineStyle(5, e.color, ea).strokeRect(cfg.wallX - 14 - prog * 6, 8, 28 + prog * 12, H - 16);
    else if (e.kind === 'bolt' && e.hops) {
      g.lineStyle(3, e.color, ea); g.beginPath(); g.moveTo(e.x, e.y);
