...
(function initCharPanel(){
  ...
  const ATTR_POOL = {
    ...
    '攻击力':{desc:'所有伤害的计算基数',affixes:['攻击力加成','攻击力强化','攻击力增幅','攻击加成','攻击增幅']},
  ...
  /* ---- 词缀 → 属性归属 + 类型（flat 直接加进基础值 / pct 作为乘区百分比） ----
   *  数值型基础属性（攻击力/生命值/护甲值/护盾值/多重射击/充能速度/暴击伤害）上的词缀默认 flat，
   *  即「先叠加进基础值」；但「攻击力」上的 攻击加成/攻击增幅 是乘区百分比（攻击力加成先加、攻击加成后乘）。
   *  其余归属到「百分比乘区属性」的词缀一律 pct。 */
  const AFFIX_TO_ATTR = {};
  const AFFIX_KIND = {};
  const FLAT_KINDS = {flat:1, count:1, charge:1, critdmg:1};
  const ATK_PCT_AFFIX = {'攻击加成':1, '攻击增幅':1}; // 攻击力上的乘区词缀
  Object.keys(ATTR_POOL).forEach(function (a) {
    const kd = (ATTR_DEFS[a] && ATTR_DEFS[a].kind) || 'pct';
    (ATTR_POOL[a].affixes || []).forEach(function (af) {
      AFFIX_TO_ATTR[af] = a;
      if (a === '攻击力' && ATK_PCT_AFFIX[af]) AFFIX_KIND[af] = 'pct';
      else if (FLAT_KINDS[kd]) AFFIX_KIND[af] = 'flat';
      else AFFIX_KIND[af] = 'pct';
    });
  });
  ...
  const EQUIP_ITEMS = [
    {id:'w1', name:'寒霜短刃', icon:'🗡️', rarity:'rare', slot:'武器',
      ...
      attrs:{'攻击力加成':25,'攻击加成':25,'冰霜增幅':30,'暴击率':5},
  ...
  // pctBonusOf: 该属性上所有 pct 词缀之和（单位 %，作为乘区，例：攻击加成 +25%）
  ...
  // getVal:
  //   数值型属性 = (基础值 + Σflat) × (1 + Σpct/100)
  //     例：攻击力 100，攻击力加成+25(flat) → 100+25=125；攻击加成+25%(pct) → 125×1.25=156.25
  //   pct 属性 = 基础(通常0) + Σpct%（独立乘区）
  function getVal(key){
    const d = ATTR_DEFS[key];
    if (!d) return 0;
    if (d.kind === 'flat' || d.kind === 'count' || d.kind === 'charge' || d.kind === 'critdmg') {
      return (d.base + flatBonusOf(key)) * (1 + pctBonusOf(key) / 100);
    }
    return d.base + pctBonusOf(key);  // pct 属性（基础通常 0）
  }
  ...
  window.flatBonusOf = flatBonusOf; // flat 词缀之和（直接加进基础值）
  window.pctBonusOf = pctBonusOf;   // pct 词缀之和（乘区百分比）
  ...
  function tryInit(){
    ...
    function showTip(target){
      ...
      const baseRow = hasBaseValue(k)
        ...
        + '<div class="baseRow"><b>' + displayVal(k) + '</b></div>'
        : '';
      tip.innerHTML =
        '<h5>' + k + '</h5>' +
