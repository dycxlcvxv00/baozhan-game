...
(function () {
  ...
  function renderTree(id) {
    ...
    /* 关键修复：iframe 只挂载一次。
     * 若每次渲染都 innerHTML='' + appendChild，iframe 从 DOM 移除再插回会触发浏览器重新导航
     * （强制重新加载）→ 每次打开面板专精树都闪白/动画重播，看起来像"缩放一下"。
     * 改为：已挂载则复用（零重载），占位技能时仅 display:none 隐藏 iframe。 */
    if (f.parentNode !== host) {
      host.innerHTML = '';
      host.appendChild(f);
    }
    if (s && s.id === 'iceLance') {
      ...
      const oldPh = host.querySelector('.treePlaceholder');
      if (oldPh) oldPh.remove();
      f.style.display = '';
      fitFrame();
    } else {
      f.style.display = 'none';
      let ph = host.querySelector('.treePlaceholder');
      if (!ph) {
