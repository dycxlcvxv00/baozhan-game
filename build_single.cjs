 ...
 * 产出：index-h-single.html（双击即玩）+ index.html（发布入口，内容相同）
 *
...
const OUT_SINGLE = path.join(ROOT, 'index-h-single.html');
...
fs.writeFileSync(OUT_SINGLE, html);
fs.writeFileSync(OUT_PUB, html);

const kb = (n) => (n / 1024).toFixed(1) + ' KB';
console.log('[build] 已生成：');
console.log('  index-h-single.html  ' + kb(fs.statSync(OUT_SINGLE).size));
console.log('  index.html           ' + kb(fs.statSync(OUT_PUB).size));
